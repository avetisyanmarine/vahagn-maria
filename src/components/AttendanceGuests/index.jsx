...
const AttendanceGuests = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await fetch(
        "https://script.google.com/macros/s/AKfycbwWBEOlpTqxHB-WemPVJ407vvqAXDuEJD2aXyzp-6ijOm-7kRBqtFLtwWJ3HzouU6jtwg/exec",
        {
          method: "POST",
          mode: "no-cors",
          headers: {
    ...
    <Wrapper>
      ...
      <Form onSubmit={handleSubmit}>
        ...
        <FormGroup>
          ...
          <OptionGrid>
            {[
              { value: "Հարսի կողմից", label: "Հարսի" },
              { value: "Փեսայի կողմից", label: "Փեսայի" },
            ].map((option) => (
              <OptionButton
                key={option.value}
                type="button"
